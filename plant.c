...
//LOOP ATTRIBUTES
	for (i=0;i<ip_column_count;i++)
	{
		...
		item_target = (int **) malloc(1 * sizeof(int * ));
...
//CHECK IF ITEM ALREADY EXISTS
				...
				if (g_lines[ip_subset[j]][ip_column[i]] == item[k])
				{
...
//INITIALIZE TARGET THAT BELONG TO THE ITEM
				...
				*/
				item_target = (int **) realloc(item_target, (k+1) * sizeof(int * ));
				item_target[k] = (int *) malloc(1 * sizeof(int));
				item_target_count=(int *) realloc(item_target_count,(k+1) * sizeof(int));
				item_target_count[k]=0;
				item_target_line_count=(int **) realloc(item_target_line_count,(k+1) * sizeof(int *));
				item_target_line_count[k]=(int *) malloc(1 * sizeof(int));
...
//CHECK IF TARGET ALEADY EXISTS
				...
				if (g_lines[ip_subset[j]][g_target] == item_target[k][l])
				{
//ADD TARGET INFO
...
//INITIALIZE TARGET INFO
				...
				item_target[k] = (int *) realloc(item_target[k],(l+1) * sizeof(int));
				item_target[k][l] = g_lines[ip_subset[j]][g_target];
				item_target_count[k]++;
...
//CALCULATE HIGH OCCURING TARGET FOR EACH BRANCH
		...
		if (n_gain==-1)
		{
			printf("****Uncertain leaf node, please check parent node for high probable target value****");
		...
		else
		{
			...
			//printf("if %s is %s ,then",g_headers[split_column],n_item[i]);
			//fprintf(op,"if %s is %s ,then",g_headers[split_column],n_item[i]);
			printf("if %s is %d ,then",g_headers[split_column],n_item[i]);
...
//CHECK IF MULTIPLE BRANCH WITH NO UNCERTAINITY AND HAS FURTHER LEVELS
		if (ip_column_count!=1 && (n_item_target_count[i]>1) && n_gain!=-1.0)
		{
			...
			for (j=0;j<n_item_target_count[i];j++)
			{
				//printf("%s-%.2f%;",n_item_target[i][j],((float)n_item_target_line_count[i][j]*100)/n_item_line_count[i]);
				//fprintf(op,"%s-%.2f%;",n_item_target[i][j],((float)n_item_target_line_count[i][j]*100)/n_item_line_count[i]);
				printf("%d-%.2f%;",n_item_target[i][j],((float)n_item_target_line_count[i][j]*100)/n_item_line_count[i]);
